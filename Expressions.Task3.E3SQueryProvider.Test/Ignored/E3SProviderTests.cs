...
namespace Expressions.Task3.E3SQueryProvider.Test.Ignored
{
    ...
    public class E3SProviderTests
    {
        ...
        {
            IEnumerable<EmployeeEntity> res = searchService.Value
                .SearchFts<EmployeeEntity>("workstation:(EPRUIZHW0249)", 0, 1);

            foreach (EmployeeEntity emp in res)
            {
        ...
        {
            IEnumerable res = searchService.Value
                .SearchFts(typeof(EmployeeEntity), "workstation:(EPRUIZHW0249)", 0, 10);

            foreach (EmployeeEntity emp in res.OfType<EmployeeEntity>())
            {
                _testOutputHelper.WriteLine("{0} {1}", emp.NativeName, emp.StartWorkDate);
            }
        }
        ...
        public void WithProvider()
        {
            ...
            foreach (EmployeeEntity emp in employees.Where(e => e.Workstation == "EPRUIZHW0249"))
            {
                _testOutputHelper.WriteLine("{0} {1}", emp.NativeName, emp.StartWorkDate);
            }
        }
